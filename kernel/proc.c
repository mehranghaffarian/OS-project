...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    bool targetProcessIsFound = false;
    int smallestPBirthTick = ticks;
    ...
    {
      acquire(&p->lock);
      if (p->state == RUNNABLE && p->pBirthTick < smallestPBirthTick && p->pid != 0 && p->pid != 1)
      {
        smallestPBirthTick = p->pBirthTick;
        targetProcess = p;
        targetProcessIsFound = true;
      }
      release(&p->lock);
    }

    if (targetProcessIsFound)
    {
...
int getProcTick(int targetProcessID)
{
  uint xticks;
...
// gets the sysInfo
int getSysInfo(uint64 sysInformation)
{
